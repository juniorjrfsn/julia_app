...
function load_pretrained_model(model_filepath::String, config_filepath::String)
    ...
    try
        ...
        return model_state, original_person_names, config
    catch e
...
# Modify model architecture for new classes
function expand_model_for_incremental(original_model, original_num_classes::Int, new_num_classes::Int)
    ...
    feature_layers = model_layers[1:end-1]  # All layers except final Dense
    
    # Get the size of the penultimate layer (input to final classification)
    penultimate_layer = model_layers[end-2]  # Dense layer before final
    if isa(penultimate_layer, Dense)
        feature_size = size(penultimate_layer.weight, 1)
    else
        error("Esperado camada Dense antes da classificação final")
    end
    
    # Create new final classification layer
    new_final_layer = Dense(feature_size, new_num_classes)
    
    # Initialize new weights: copy old weights for existing classes, random for new classes
    old_final_layer = model_layers[end]
    if isa(old_final_layer, Dense)
        # Initialize all weights randomly first
        new_final_layer.weight[:, :] = randn(Float32, new_num_classes, feature_size) * 0.01f0
        new_final_layer.bias[:] = zeros(Float32, new_num_classes)
        
        # Copy weights for existing classes
        new_final_layer.weight[1:original_num_classes, :] = old_final_layer.weight
        new_final_layer.bias[1:original_num_classes] = old_final_layer.bias
        
        println("   - Copiados pesos para $original_num_classes classes existentes")
        println("   - Inicializados pesos para $(new_num_classes - original_num_classes) classes novas")
    else
        error("Camada final não é Dense")
    end
    
    # Combine all layers
    expanded_model = Chain(feature_layers..., new_final_layer)
