...
function normalizar_dados(dados)
    ...
    dados_normalizados = similar(dados)
...
dados_normalizados = [(normalizar_dados([d[1]])[1], d[2]) for d in dados]
...
function obter_entradas()
    ...
    entradas_normalizadas = normalizar_dados([peso, altura, idade, atividade_fisica])
